...
const createLeaveApplication = async (req, res, next) => {
    try {
        const { userId, application, leaveType, companyId, clientId } = await sanitizeBody(req.body);
        const result = await createLeaveApplicationFn({ userId, application, leaveType, companyId, clientId });
