...
menuMasterSchema.methods.softDelete = async function ({userId}) {
    try {
       ...
       const result =  await this.save()
        return { status: true, message: "Menu deleted successfully", data: result }
    } catch (error) {
...
menuMasterSchema.methods.edit = async function ({userId, updatedData, menumasterId}) {
    try {
        const updatedMenu = await this.constructor.findOneAndUpdate(
            { _id: menumasterId, deletedAt: null }, 
