const getDocumentPropertiesAll = require("../../model/services/documentProperties/getAllDocumentProperties");
const getByIdDocumentProperties = require("../../model/services/documentProperties/getByIdDocumentsProperties");
const sanitizeBody = require("../../utils/sanitizeBody")


const getAllDocumentProprties = async (req, res, next) =>{
    try {
        ...
        const result = await getDocumentPropertiesAll({ page, perPage, searchKey, clientId})
...
const getDocumentPropertiesById = async (req, res, next) =>{
    try {
        const data = await sanitizeBody(req.params)
        const {clientId, id} = data
        const result = await getByIdDocumentProperties({clientId, id})
        return res.status(200).json({status: result.status, message: result.message, data: result.data})
    } catch (error) {
        next(error)
    }
}
module.exports = {getAllDocumentProprties, getDocumentPropertiesById}