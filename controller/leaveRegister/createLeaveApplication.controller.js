...
const createLeaveApplication = async (req, res, next) => {
    try {
        const { userId, companyId, application, leaveType, clientId } = await sanitizeBody(req.body);
        const result = await createLeaveApplicationFn({ userId, companyId, application, leaveType, clientId });
