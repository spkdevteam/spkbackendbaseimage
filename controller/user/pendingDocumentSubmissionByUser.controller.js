const documentPendingByUser = require("../../model/services/user/documentPendingByUser");
const sanitizeBody = require("../../utils/sanitizeBody")

const pendingDocumentSubmissionByUser = async (req, res, next) =>{
    try {
        const {clientId, role} = await sanitizeBody(req.params)
        ...
        const result = await documentPendingByUser({ page, perPage, searchKey, clientId, role})
        return res.status(200).json({status: result?.status, message: result?.message, data: result?.data, metaData: result?.metaData})
    } catch (error) {
        next(error)
    }
}
module.exports = pendingDocumentSubmissionByUser