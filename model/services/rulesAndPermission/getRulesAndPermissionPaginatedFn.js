...
const getRulesAndPermissionPaginatedFn = async ({ page = 1, perPage = 10, searchKey="", clientId}) => {
    try {
        ...
        if (searchKey.trim()) {
            ...
            if (isNaN(searchKey)) {
                searchQuery = {
                    $or: [
                        { rulesName: { $regex: `^${escapedSearchKey}`, $options: "i" }, deletedAt: null },
                    ]
                }
            }
        };
...
module.exports = getRulesAndPermissionPaginatedFn;