...
const editOneDutyAndResponsibility = async (req, res, next) => {
    try {
        const { deptName, designation, rules, documents, companyId, clientId } = await sanitizeBody(req.body);
        const result = await editOneDutyAndResponsibilityFn({ deptName, designation, rules, documents, companyId, clientId });
