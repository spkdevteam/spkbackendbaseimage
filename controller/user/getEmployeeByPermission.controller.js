...
const getUserByPermission = require("../../model/services/user/getUserByPermission");
const sanitizeBody = require("../../utils/sanitizeBody")

const getEmployeeByPermission = async (req, res, next) =>{
    try {
        const {clientId, roleId, permissionId} = await sanitizeBody(req.params)
        ...
        const result = await getUserByPermission({ page, perPage, searchKey, clientId, roleId, permissionId})
        return res.status(200).json({status: result.status, message: result.message, data: result?.data, metaData: result?.metaData })
    } catch (error) {
        next(error)
    }
}
module.exports = getEmployeeByPermission